...
def find_mat_files(subject_dir: str):
    # supports .mat and weird .mat.mat
...
def contiguous_segments(rep: np.ndarray):
    """Return list of (start, end) for contiguous blocks where rep is constant."""
    if rep.ndim != 1:
        rep = rep.reshape(-1)
...
def window_emg_by_repetition(X, y, rep, fs=2000, win_ms=150, overlap=0.5):
    win_len = int(fs * win_ms / 1000)      # 150ms -> 300 samples
    step = int(win_len * (1 - overlap))    # 50% overlap -> 150
    Xw, yw = [], []
    ...
    for (a, b) in contiguous_segments(rep):
        ...
        for start in range(a, b - win_len + 1, step):
            ...
            x_win = X[start:end].T                         # (12, win_len)
            label = y[start + win_len // 2]                # center label
